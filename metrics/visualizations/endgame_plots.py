...
def add_impact_points(fig, endgame_data, truth_data, include_truth=True):
    ...
    for seed, seed_data in endgame_data.groupby('Seed'):
        ...
        fig.scatter(x='lat', y='lon', color=colors[int(seed % len(colors))], size=20,
                    alpha=0.6, source=cds, legend_label=f'{seed}')
    ...
    if include_truth:
        ...
        fig.scatter(x='lat', y='lon', color='red', marker='triangle', size=20, source=truth_cds)
...
def make_endgame_plot(endgame_data,  output_location=None, width=750, height=750):
    ...
    truth_data = truth_data[truth_data['Time'] == 119]
